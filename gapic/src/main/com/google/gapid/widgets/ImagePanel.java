...
public class ImagePanel extends Composite {
  ...
  private static class ImageComponent extends Composite {
    ...
    public Pixel getPixel(Point point) {
      ...
      for (int i = 0; i < images.length; i++) {
        ...
        VecD imageTexel = imageNormalized.scale(0.5).add(0.5, 0.5, 0.0).multiply(tileSize);
    ...
    public void zoom(int amount, Point cursor) {
      ...
      if (cursor != null) {
        ...
        viewOffset = viewOffset
            .subtract(imageNormDelta.multiply(tileSize).scale(0.5))
            .clamp(viewOffsetMin, viewOffsetMax);
      } else {
        setScale(scale);
      }
    ...
    private void setScale(double scale) {
      ...
      VecD viewSizeSubBorder = viewSize.subtract(BORDER_SIZE.scale(2));
  ...
  private static class ImageScene implements Scene<SceneData> {
    ...
    public void update(Renderer renderer, SceneData data) {
      ...
      Set<Image> newSet = Sets.newHashSet(data.images);
      ...
      this.textures = new Texture[data.images.length];
