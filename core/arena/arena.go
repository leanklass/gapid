...
// NewArena constructs a new arena.
// You must call Dispose once the arena to free the arena object and any arena-
// owned allocations.
func NewArena() Arena {
	return Arena{Pointer: unsafe.Pointer(C.native_arena_create())}
}

// Dispose destructs and frees the arena and all arena-owned allocations.
func (a Arena) Dispose() {
	C.native_arena_destroy((*C.native_arena)(a.Pointer))
}
...
func (a Arena) Allocate(size, alignment int) unsafe.Pointer {
	return C.native_arena_alloc((*C.native_arena)(a.Pointer), C.uint32_t(size), C.uint32_t(alignment))
}
...
func (a Arena) Reallocate(ptr unsafe.Pointer, size, alignment int) unsafe.Pointer {
	return C.native_arena_realloc((*C.native_arena)(a.Pointer), ptr, C.uint32_t(size), C.uint32_t(alignment))
}

// Free releases the memory at ptr, which must have been previously allocated by
// this arena.
func (a Arena) Free(ptr unsafe.Pointer) {
	C.native_arena_free((*C.native_arena)(a.Pointer), ptr)
}
